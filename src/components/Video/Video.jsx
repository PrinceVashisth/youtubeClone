...
const [channel,setChannel]= useState({});
  useEffect(()=>{
    const FetchChennel=async()=>{
     const res = await axios.get(`https://youtube-api-1.onrender.com/api/channel?userId=${props.userId}`);
     setChannel(res.data);
    }
    FetchChennel();
   },[props]);
...
const ViewsHandeller=async()=>{
  user ?await axios.put(`https://youtube-api-1.onrender.com/api/user/video/${props._id}`,{userId:user._id}): await axios.put(`https://youtube-api-1.onrender.com/api/user/video/${props._id}`,{userId:user});
}
...
const ReportHandeller=async()=>{
 const res = await axios.put(`https://youtube-api-1.onrender.com/api/user/report/video/${props._id}`,{userId:user._id});
